...
def run(license_plate_config, character_config, save_dir, visualize, save_img): 
    ...
    for _, image, _, count in dataset:
        ...
        boxes = get_specific_classes(boxes, cls = [2, 3])
        ...
        for tracking_box in tracking_boxes:
            ...
            perspective_image = perspective_transform(enhanced_img) 
            
            character_boxes = character_detector.detect(perspective_image)
            
            new_boxes = convert_boxes_to_ratio(character_boxes, perspective_image.shape)
            result = get_value(new_boxes)

            plate_img = character_detector.visualize(perspective_image, character_boxes)
            ...
            if save_img:
                ...
                perspective_img_path = os.path.join(track_dir, str(count) + "_perspective.png")

                cv2.imwrite(img_path, plate_img)
                cv2.imwrite(enhanced_img_path, enhanced_img)
                cv2.imwrite(perspective_img_path, perspective_image)
